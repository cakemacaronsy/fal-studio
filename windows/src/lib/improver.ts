// Port of Sources/Services/PromptImprover.swift — builds the LLM instructions
// for the ✨ Improve Prompt button. Guidance is distilled from each model's
...
import { MediaKind, ModelSpec } from "./types";

export const defaultImproveLLM = "deepseek/deepseek-chat";

/** Models from Chinese labs understand Chinese prompts natively; for them a
 *  Chinese input stays Chinese. International models get English. */
function isChineseCapable(spec: ModelSpec): boolean {
  const e = spec.endpoint;
  return (
    e.includes("bytedance") || e.includes("kling") || e.includes("minimax") || e.includes("/wan/")
  );
}
...
type Family =
  | "seedance20" | "seedance25" | "minimax" | "kling" | "grok" | "grokVideo"
  | "wan" | "ltx" | "seedream" | "gptImage" | "custom";

function familyOf(spec: ModelSpec): Family {
  const endpoint = spec.endpoint;
  if (endpoint.includes("seedance-2.5")) return "seedance25";
  if (endpoint.includes("seedance")) return "seedance20";
  if (endpoint.includes("minimax")) return "minimax";
  if (endpoint.includes("kling")) return "kling";
  if (endpoint.includes("grok-imagine-video")) return "grokVideo";
  if (endpoint.includes("grok-imagine")) return "grok";
  if (endpoint.includes("/wan/")) return "wan";
  if (endpoint.includes("ltx")) return "ltx";
  if (endpoint.includes("seedream")) return "seedream";
  if (endpoint.includes("gpt-image")) return "gptImage";
  return "custom";
}

export function improverSystemPrompt(
  spec: ModelSpec,
  jsonMode: boolean,
  userPrompt: string
): string {
  ...
  rules.push(
    `You rewrite prompts for the AI ${spec.kind === "video" ? "video" : "image"} generation model "${spec.displayName}" (endpoint ${spec.endpoint}). Return ONLY the rewritten prompt — no preamble, no explanations, no markdown fences.`
  );

  if (isChineseCapable(spec) && containsCJK(userPrompt)) {
    rules.push(
      "The user wrote in Chinese and this model understands Chinese natively: write the improved prompt in Traditional Chinese, keeping any quoted dialogue exactly as given."
    );
  } else {
    rules.push(
      "Write the improved prompt in English (translate the user's idea faithfully if it isn't English). Keep any quoted dialogue in its original language inside double quotes."
    );
  }

  rules.push(guidance(familyOf(spec), spec.kind));

  if (jsonMode) {
    rules.push(structuredTemplate(familyOf(spec), spec.kind));
  } else {
    rules.push(
      "Output one flowing prompt. Preserve the user's core subject and intent exactly — enrich and direct, never replace. Respect the model's word-count sweet spot noted above."
    );
  }
  return rules.join("\n\n");
}
...
function guidance(family: Family, kind: MediaKind): string {
  switch (family) {
    case "seedance20":
      return `Seedance 2.0 official guide — write a shot brief in plain natural language (50–150 words), like instructions to a director of photography:
...
7. BAN the slop words: beautiful, stunning, cinematic, epic, masterpiece — they give the model nothing to point a camera at.`;
    case "seedance25":
      return `Seedance 2.5 official guide — everything from Seedance 2.0 (shot-brief structure, physical acting with body-weight consequences, quoted lip-synced dialogue with blocking, explicit diegetic audio, no slop adjectives) PLUS the long-shot rules:
...
4. OCCLUSION CONTINUITY: when a subject passes behind something, restate their identity on re-emergence ("the same woman, same green coat, emerges from the other side").`;
    case "minimax":
      return `MiniMax H3 official guide:
...
7. Favor composition instructions over story narration; assign explicit jobs to any reference images.`;
    case "kling":
      return `Kling 3.0 official guide — think in SHOTS, not clips:
...
5. Describe camera movement explicitly for every shot; generic motion language is the main failure mode.`;
    case "grok":
      return `Grok Imagine v2 — photographic natural language. Cover five things concretely:
...
Anti-slop: visual facts over vague praise — replace "stunning/beautiful/masterpiece" with observable specifics ("overcast daylight", "50mm feel", "realistic skin texture").`;
    case "grokVideo":
      return `Grok Imagine Video 1.5 — natural-language shot direction (6–15 s clips, native audio):
...
5. No slop words (cinematic, stunning, epic) — observable specifics only.`;
    case "wan":
      return `Wan 2.7 — cinematic natural language:
...
5. Avoid adjective piles; use observable visual facts and real camera vocabulary.`;
    case "ltx":
      return `LTX-2.5 (Lightricks) — natural-language shot direction with native multishot and synchronized audio:
...
5. Observable specifics over adjectives; no filler words like cinematic/stunning.`;
    case "seedream":
      return `Seedream 5.0 Pro official guide — brief it like a photographer or a designer:
...
6. Don't overload one prompt with dense multi-region layouts — fewer, clearer regions win.`;
    case "gptImage":
      return `GPT Image 2 official guide — structured brief with anti-slop rules:
...
function structuredTemplate(family: Family, kind: MediaKind): string {
  switch (family) {
    case "seedance25":
      return `Format the output using Seedance 2.5's production-note sections, in this order, as labeled plain-text lines (no JSON, no markdown):
...
CONSTRAINTS: what must not happen.`;
    case "seedance20":
      return `Format the output as an explicit multi-shot brief in plain text (no JSON): number each shot ("Shot 1 — …"), one action and one camera move per shot, join with "cut to", then a final AUDIO line naming diegetic sounds/room tone/dialogue in double quotes.`;
    case "minimax":
      return `Format the output as a timed shot list in plain text (no JSON): lines of [0–2s] …, [2–4s] …, each with framing + action + camera; then an AUDIO line and a CONSTRAINTS line of explicit negatives.`;
    case "kling":
      return `Format the output as labeled shots in plain text (no JSON): "Shot 1 (framing): …" per shot, characters introduced by name up front, dialogue as [Name: tone]: "line", sequence controlled with linking words.`;
    case "gptImage":
      return `Format the output using GPT Image 2's official sections as labeled lines (no JSON):
Scene: …
Subject: …
Important details: …
Use case: …
Constraints: …`;
    case "seedream":
      return `Format the output as a design/photo brief with labeled lines (no JSON): Subject / Camera / Light / Finish for photos, or Grid / Regions (one line per region, copy in double quotes with placement) / Style for layouts.`;
    case "grokVideo":
    case "wan":
    case "ltx":
